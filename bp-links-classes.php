...
class BP_Links_Link {
	...
	static function popularity_recalculate_all() {
		global $wpdb, $bp;
	...
	static function link_exists( $slug_or_id ) {
		global $wpdb, $bp;
	...
	static function get_by_columns_filtered( $args, $sort_columns = array() ) {
		global $wpdb, $bp;
	...
	static function get_popular( $args ) {
	...
	static function get_total_link_count() {
		global $wpdb, $bp;
	...
	static function get_total_link_count_for_user( $user_id = false ) {
		global $bp, $wpdb;
	...
	static function get_status_sql( $link_owner_user_id = false, $format_string = '%s' ){
		global $bp;
...
class BP_Links_Vote {
	 ...
	 * @return boolean
	 */
	function __construct( $link_id = false, $user_id = false ) {
		if ( $link_id && $user_id ) {

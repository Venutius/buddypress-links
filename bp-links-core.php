...
function bp_links_setup_nav() {
	...
	if ( bp_is_links_component() ) {
		...
		} else if ( !bp_is_my_profile() && !$bp->is_single_item && isset( $bp->displayed_user->id ) ) {
...
function bp_links_add_meta() {
	...
	if ( isset( $bp->links->current_link->description ) ) {
		$description = $bp->links->current_link->description;
	} else {
		$description = '';
	}
	if ( $bp->is_single_item) {
		printf(
			...
			apply_filters( 'bp_links_add_meta_description_single_item', $description )
		);
	}
}
...
function bp_links_action_personal_links_feed() {
	...
	if ( !bp_is_activity_component() || !bp_displayed_user_id() || $bp->current_action != 'my-links' || $bp->action_variables[0] != 'feed' )
		return false;
...
function bp_links_total_links_for_user( $user_id = false ) {
	...
	if ( !$user_id )
		$user_id = ( isset( $bp->displayed_user->id ) ) ? $bp->displayed_user->id : $bp->loggedin_user->id;
